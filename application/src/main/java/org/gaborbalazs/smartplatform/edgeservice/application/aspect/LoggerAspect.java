...
public class LoggerAspect {
    ...
    Object aroundMethods(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        logger.debug(BEFORE + "{}" + BRACKET_PREFIX + "{}" + BRACKET_SUFFIX, joinPoint.getSignature().getName(), args.stream().map(Object::toString).collect(Collectors.joining(", ")));
        Object response = joinPoint.proceed();
        logger.debug(AFTER + "{}" + BRACKET_PREFIX + "{}" + BRACKET_SUFFIX, joinPoint.getSignature().getName(), response);
        return response;
    }
}
